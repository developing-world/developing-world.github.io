...
def ccompiler():
    check = ""
    return render_template('ccompiler.html', check=check)
...
def Run():
    ...
    return render_template('ccompiler.html', code=code, input=inp, output=output, check=check)
...
def complier_output(code, inp, chk):
    ...
    s = subprocess.run(['g++', '-o', 'new', 'Try.c'],
...
def cpp():
    check = ""
    return render_template('cpp.html', check=check)


# route for the submit page to show the output/error of the c program.
@app.route('/Runn', methods=['GET', 'POST'])
def Runn():
    if request.method == 'POST':
        ...
        output = complier_output(code, inp, chk)
    # return render_tempelate to
    return render_template('cpp.html', code=code, input=inp, output=output, check=check)
...
def complier_output(code, inp, chk):
    ...
    os.close(fd)
    # Compiling the c program file and retrieving the error if any.
    s = subprocess.run(['g++', '-o', 'new', 'Try.cpp'],
                       stderr=PIPE, shell=True)
    # storing the value returned by return code.
    check = s.returncode
    # checking whether program compiled succesfully or not.
